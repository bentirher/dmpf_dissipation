...
# -----------------------------------------------------------------------------
#
# `maxdim_G` is separate from `maxdim` because G reaches Phi only pre-multiplied
# by two factors of delta. The error analysis says an error eta_G on G arrives at
# Phi as eta_G * ||delta||^2, i.e. RELATIVE error eta_G on the answer.
#
# MEASURED CAVEAT: that is relative-in / relative-out, NOT "G can be sloppy".
# maxdim_G = maxdim/2 at n=4, chi=256 gave dc = 9.4e-4; maxdim_G = maxdim gave
# 1.0e-4. Leave maxdim_G = -1 (meaning "same as maxdim") unless you have a
# specific reason and have measured the cost.

function build_Phi_pair(n, J, gammas, t, k_i::Int, k_j::Int, k0::Int,
                        lsites::LiouvilleSites;
                        cutoff=1e-12, maxdim=128, maxdim_G=-1,
                        order::Int=2, order_ref::Int=2, dissipation::Bool=true,
    ...
    # maxdim_G < 0 means "same as maxdim". Setting maxdim_G below maxdim is NOT
    # free: at n=4 a run with maxdim=256, maxdim_G=128 gave dc = 9.4e-4 where
    # maxdim_G=256 gave 1.0e-4. G needs the same RELATIVE accuracy you want in
    # the answer -- the "G can be sloppy" framing was too loose.
    maxdim_G = maxdim_G < 0 ? maxdim : maxdim_G
