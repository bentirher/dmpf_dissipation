...
gammas = fill(0.0, n)        # <-- no dissipation
t      = 3.0
k      = 8
cutoff = 1e-12
...
dt = t / k
step_mpo     = get_open_step_MPO(n, J, gammas, dt, lsites, cutoff, maxdim; order=2, dissipation=false)
step_dag_mpo = get_open_step_MPO_dag(n, J, gammas, dt, lsites, cutoff, maxdim; order=2, dissipation=false)
...
# =============================================================================
println("[$(now())] === MOC (lockstep) ==="); flush(stdout)
F = deepcopy(Id)
for m in 1:k
    global F
...
# =============================================================================
println("[$(now())] === PLAIN (forward-then-backward) ==="); flush(stdout)
F = deepcopy(Id)
step = 0
# k forward steps
for m in 1:k
...
# write results
# =============================================================================
outdir = "moc_results"; mkpath(outdir)
